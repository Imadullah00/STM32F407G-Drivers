...
/*************************Check for TC flag ********************************************/
	...
	temp2 = pUSARTHandle->pUSARTx->USART_SR & ( 1 << USART_CR1_TCIE);
	...
	if(temp1 && temp2 )
	{
		...
		if ( pUSARTHandle->TxBusyState == USART_BUSY_IN_TX)
		{
			...
			if(! pUSARTHandle->TxLen )
			{
				...
				uint32_t dummy = pUSARTHandle->pUSARTx->USART_DR;
				(void) dummy;
...
/*************************Check for IDLE detection flag ********************************************/
	...
	if(temp1 && temp2)
	{
		...
		uint32_t dummy3 = pUSARTHandle->pUSARTx->USART_SR;
		dummy3 = pUSARTHandle->pUSARTx->USART_DR;
		(void) dummy3;
...
//The below code will get executed only if multibuffer mode is used.
	...
	temp2 =  pUSARTHandle->pUSARTx->USART_SR & ( 1 << USART_CR3_EIE) ;
	...
	if(temp2 )
	{
		...
		if(temp1 & ( 1 << USART_SR_FE))
		{
			...
			*/
			uint32_t dummy2 = pUSARTHandle->pUSARTx->USART_SR;
			dummy2 = pUSARTHandle->pUSARTx->USART_DR;
			(void) dummy2;
			USART_ApplicationEventCallback(pUSARTHandle,USART_ERREVENT_FE);
		}
